$(function () {
    ...
    // 定义美化时间的过滤器
    // 使用方法  {{$value.pub_date|dataFormat}}
    ...
    function initCate() {
        $.ajax({
            ...
            success: function (res) {
                ...
                // 此时会发现下拉菜单并没有内容，这是layui渲染机制导致的，此时需要重新渲染表单区
                form.render()
            }
        })
    }
    ...
    function renderPage(total) {
        ...
        laypage.render({
            ...
            jump: function (obj, first) {
                ...
                // 但是把 initTable() 放在这里会陷入死循环
                // 所以此时需要用到第二个参数 first
                console.log(first);
                // 如果 first 的值为 True ，则说明 jump 是以第二方式被触发的，此时不能调用 initTable（）
                if (!first) {
                    initTable()
                }
            }
        ...
        layer.confirm('确认删除?', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                ...
                success: function (res) {
                    ...
                    // 如果没有剩余的数据了，则让页码值 -1 之后，在重新调用 initTable 方法
                    if (len === 1) {
